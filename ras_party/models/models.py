...
class Business(Base):
    ...
    attributes = relationship(
        "BusinessAttributes",
        backref="business",
        order_by="desc(BusinessAttributes.created_on)",
        lazy="joined",
    )
    ...
    def to_party(business_data):
        ...
        party, attrs = partition_dict(
            business_data, ["sampleUnitRef", "sampleUnitType", "id", "sampleSummaryId"]
        )
    ...
    def from_party_dict(party):
        ...
        b = Business(
            ...
            business_ref=party["sampleUnitRef"],
        )
        ba = BusinessAttributes(
            business_id=b.party_uuid, sample_summary_id=party["sampleSummaryId"]
        )
    ...
    def add_versioned_attributes(self, party):
        ba = BusinessAttributes(
            business_id=self.party_uuid, sample_summary_id=party["sampleSummaryId"]
        )
        ba.attributes = party.get("attributes")
        ...
        try:
            return next(
                (
                    attributes
                    for attributes in self.attributes
                    if attributes.collection_exercise
                )
            )
        except StopIteration:
            logger.error(
                "No active attributes for business",
                reference=self.business_ref,
                status=404,
            )
            raise NotFound(
                "Business with reference does not have any active attributes."
            )
...
class BusinessAttributes(Base):
    ...
    name = Column(
        Text
    )  # New columns placed at end of list in case code uses positional rather than named references
...
class BusinessRespondent(Base):
    ...
    status = Column(
        "status",
        Enum(BusinessRespondentStatus),
        default=BusinessRespondentStatus.ACTIVE,
    )
...
class PendingEnrolment(Base):
    ...
    __table_args__ = (ForeignKeyConstraint(["respondent_id"], ["respondent.id"]),)
...
class Enrolment(Base):
    ...
    business_respondent = relationship(
        "BusinessRespondent", back_populates="enrolment", lazy="joined"
    )
...
class PendingShares(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(["business_id"], ["business.party_uuid"]),
        ForeignKeyConstraint(["shared_by"], ["respondent.party_uuid"]),
        UniqueConstraint(
            "email_address", "business_id", "survey_id", name="u_constraint"
        ),
    )
