...
@with_query_only_db_session
def get_respondents_by_name_and_email(
    ...
    :return: Respondents
    """
    respondents, record_count = query_respondent_by_names_and_emails(
        first_name, last_name, email, page, limit, session
    )
    return {
        "data": [respondent.to_respondent_dict() for respondent in respondents],
        "total": record_count,
    }
...
def get_respondent_by_id(respondent_id, session):
    ...
    try:
        ...
        logger.info(
            "respondent_id value is not a valid UUID", respondent_id=respondent_id
        )
        raise BadRequest(
    ...
    if not respondent:
        logger.info(
            "Respondent with party id does not exist", respondent_id=respondent_id
        )
    ...
    if respondent:
        try:
            session.query(Respondent).filter(
                Respondent.party_uuid == respondent.party_uuid
...
def delete_respondents_marked_for_deletion(session):
    ...
    for respondent in respondents:
        session.query(Enrolment).filter(
            Enrolment.respondent_id == respondent.id
        ).delete()
        session.query(BusinessRespondent).filter(
            BusinessRespondent.respondent_id == respondent.id
        ).delete()
        session.query(PendingEnrolment).filter(
            PendingEnrolment.respondent_id == respondent.id
        ).delete()
...
def _is_user_enrolled_on_survey(associations, business_id, survey_id):
    for association in associations:
        surveys = [
            v["surveyId"]
            for v in association["enrolments"]
            if v["enrolmentStatus"] == "ENABLED"
        ]
