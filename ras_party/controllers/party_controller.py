...
def parties_post(party_data, session):
    ...
    if errors:
        logger.info(
            "party schema validation failed", errors=[e.split("\n")[0] for e in errors]
        )
...
def get_party_by_id(sample_unit_type, party_id, session):
    ...
    if sample_unit_type == Business.UNIT_TYPE:
        ...
        if not business:
            logger.info(
                "Business with id does not exist", business_id=party_id, status=404
            )
        ...
        if not respondent:
            logger.info(
                "Respondent with id does not exist", respondent_id=party_id, status=404
            )
        ...
        raise BadRequest(
            f"{sample_unit_type} is not a valid value for sampleUnitType. Must be one of ['B', 'BI']"
        )


def get_party_with_enrolments_filtered_by_survey(
    sample_unit_type, party_id, survey_id, enrolment_status=None
):
    ...
    for association in party["associations"]:
        ...
        filtered_enrolments = filter_enrolments(
            association["enrolments"], survey_id, enrolment_status
        )
