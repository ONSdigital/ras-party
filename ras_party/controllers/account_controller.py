    ...
    try:
        with session.begin_nested():
            ...
            pending_enrolment = PendingEnrolment(
                case_id=case_id, respondent=respondent, business_id=business_id, survey_id=survey_id
            )
            Enrolment(business_respondent=business_respondent, survey_id=survey_id, status=EnrolmentStatus.PENDING)
...
def _change_respondent_enrolment_status(respondent, survey_id, business_id, status, session):
    ...
    enrolment = query_enrolment_by_survey_business_respondent(
        respondent_id=respondent.id, business_id=business_id, survey_id=survey_id, session=session
    )
...
def delete_respondent_password_token(respondent_id, session):
    ...
    print("query respondent")
    respondent = query_respondent_by_party_uuid(respondent_id, session)
    print("respondent queried")
    print(respondent)
    if not respondent:
        logger.info("Respondent with party id does not exist", respondent_id=respondent_id)
        raise NotFound("Respondent id does not exist")
...
def request_password_change(payload, session):
    ...
    personalisation = {"RESET_PASSWORD_URL": verification_url, "FIRST_NAME": respondent.first_name}
...
def update_verified_email_address(respondent, tran):
    ...
    def compensate_oauth_change():
        rollback_response = OauthClient().update_account(
            username=new_email_address, new_username=email_address, account_verified="true"
        )
        respondent.pending_email_address = new_email_address
...
def resend_account_email_change_verification_email_expired_token(token, session):
    ...
    logger.info("Account change email URL for party_id", party_id=str(respondent.party_uuid), url=verification_url)
    _send_account_email_change_email(
        personalisation, template="verify_account_email_change", email=email_address, party_id=respondent.party_uuid
    )
...
def add_new_survey_for_respondent(payload, tran, session):
    ...
    enrolment = Enrolment(business_respondent=business_respondent, survey_id=survey_id, status=EnrolmentStatus.ENABLED)
    ...
    # This ensures the log message is only written once the DB transaction is committed
    tran.on_success(lambda: logger.info("Respondent has enroled to survey for business", business=business_id))
...
def enrol_respondent_for_survey(respondent, session):
    ...
    pending_enrolment = session.query(PendingEnrolment).filter(PendingEnrolment.id == pending_enrolment_id).one()
    enrolment = (
...
def get_business_survey_casegroups(survey_id, business_id):
    ...
    collection_exercise_ids = [ce["id"] for ce in request_collection_exercises_for_survey(survey_id)]
    casegroups = request_casegroups_for_business(business_id)

    # Filtering casegroups by collection exercise ids
    ce_casegroup_ids = [
        casegroup["id"] for casegroup in casegroups if casegroup["collectionExerciseId"] in collection_exercise_ids
    ]
    logger.info(
        "Successfully retrieved casegroups for business and survey", survey_id=survey_id, business_id=business_id
    )
