...
def post_respondent(party, session):
    ...
    try:
        respondent = _add_enrolment_and_auth(
            business, business_id, case_id, party, session, survey_id, translated_party
        )
    except HTTPError:
...
def _add_enrolment_and_auth(
    ...
    try:
        ...
        logger.exception(
            "Party service db post respondent caused exception",
            party_uuid=translated_party["party_uuid"],
        )
        ...
        oauth_response = OauthClient().create_account(
            party["emailAddress"].lower(), party["password"]
        )
    ...
    logger.info(
        "New user has been registered via the auth-service",
        party_uuid=translated_party["party_uuid"],
    )
    return respondent
...
def _change_respondent_enrolment_status(
    ...
    logger.info(
        "Attempting to change respondent enrolment",
    ...
    if not enrolment_count:
        logger.info(
            "Informing case service of no active enrolments",
            survey_id=survey_id,
            business_id=business_id,
            respondent_id=respondent.party_uuid,
        )
...
def disable_all_respondent_enrolments(respondent_email, session):
    ...
    for enrolment in enrolments:
        _change_respondent_enrolment_status(
            respondent=respondent,
    ...
    logger.info(
        "Completed disabling respondent enrolments",
...
def get_single_respondent_by_email(email, session):
    ...
    try:
        ...
        logger.error(
            "Respondent with email does not exist", email=obfuscate_email(email)
        )
        raise NotFound("Respondent with email does not exist")
    except MultipleResultsFound:
        logger.error(
            "Multiple respondents found for email", email=obfuscate_email(email)
        )
...
def change_respondent(payload, session):
    ...
    if "change_requested_by_respondent" in payload:
        ...
        logger.info(
            "Account change email URL for party_id",
    ...
    logger.info(
        "Verification email sent for changing respondents email",
...
def change_respondent_password(payload, tran, session):
    ...
    if respondent.status != RespondentStatus.ACTIVE:
        ...
        oauth_response = OauthClient().update_account(
            username=email_address, password=new_password
        )
...
def notify_change_account_status(payload, party_id: str, session):
    ...
    if status == "ACTIVE":
        ...
        oauth_response = OauthClient().update_account(
            username=email_address, account_locked="False"
        )
        try:
            oauth_response.raise_for_status()
        except HTTPError:
            logger.error(
                "Unexpected response from auth service, unable to unlock account",
                respondent_id=str(respondent.party_uuid),
                status=oauth_response.status_code,
            )
    ...
    # Lock and notify respondent of account lock
        ...
        logger.info(
            "Unlock account via password reset url",
            url=verification_url,
            party_id=party_id,
        )
        ...
        try:
            ...
            # Note: intentionally suppresses exception
            logger.error(
                "Error sending request to Notify Gateway", respondent_id=party_id
            )
...
def update_verified_email_address(respondent, tran):
    ...
    oauth_response = OauthClient().update_account(
        username=email_address, new_username=new_email_address, account_verified="true"
    )

    if oauth_response.status_code != 201:
        logger.error(
            "Unexpected response from auth service, unable to change user email",
            respondent_id=str(respondent.party_uuid),
            status=oauth_response.status_code,
        )
    ...
    def compensate_oauth_change():
        ...
        if rollback_response.status_code != 201:
            logger.error(
                "Unexpected response from auth service, unable to rollback change to respondent email",
                respondent_id=str(respondent.party_uuid),
                status=oauth_response.status_code,
            )
...
def resend_account_email_change_verification_email_expired_token(token, session):
    ...
    """
    logger.info(
        "Attempting to resend account email change email with expired token",
        token=token,
    )
    email_address = decode_email_token(token)
    respondent = query_respondent_by_pending_email(email_address, session)
    ...
    logger.info(
        "Successfully resent account email change verification email", token=token
    )
...
def resend_verification_email_expired_token(token, session):
    ...
    """
    logger.info(
        "Attempting to resend verification email with expired token", token=token
    )
    email_address = decode_email_token(token)
    respondent = query_respondent_by_email(email_address, session)
    ...
    response = _resend_verification_email(respondent)
    logger.info(
        "Successfully resent verification email with expired token", token=token
    )
    return response
...
def _resend_verification_email(respondent):
    if respondent.pending_email_address:
        _send_email_verification(
            respondent.party_uuid, respondent.pending_email_address
        )
    else:
...
def add_new_survey_for_respondent(payload, tran, session):
    ...
    if count_enrolment_by_survey_business(survey_id, business_id, session) == 0:
        logger.info(
            "Informing case of respondent enrolled",
            survey_id=survey_id,
            business_id=business_id,
            respondent_id=respondent.party_uuid,
        )
        post_case_event(
            case_id=case_id, category="RESPONDENT_ENROLED", desc="Respondent enroled"
        )
...
def _send_email_verification(party_id, email):
    ...
    logger.info(
        "Verification URL for party_id", party_id=str(party_id), url=verification_url
    )
    ...
    try:
        ...
        # Note: intentionally suppresses exception
        logger.error(
            "Error sending verification email for party_id", party_id=str(party_id)
        )
...
def _send_account_email_change_email(personalisation, template, email, party_id):
    ...
    try:
        logger.info(
            "sending confirmation email for respondent account change",
        ...
        logger.info(
            "confirmation email for respondent account change sent",
            party_id=str(party_id),
        )
    except RasNotifyError:
        # Note: intentionally suppresses exception
...
def enrol_respondent_for_survey(respondent, session):
    ...
    pending_enrolment = (
        session.query(PendingEnrolment)
        .filter(PendingEnrolment.id == pending_enrolment_id)
        .one()
    )
    enrolment = (
    ...
    session.add(enrolment)
    logger.info(
        "Enabling pending enrolment for respondent",
        party_uuid=respondent.party_uuid,
        survey_id=pending_enrolment.survey_id,
        business_id=pending_enrolment.business_id,
    )
    ...
    if (
        count_enrolment_by_survey_business(
            enrolment.business_id, enrolment.survey_id, session
        )
        == 0
    ):
        logger.info(
        ...
        post_case_event(
            case_id=case_id, category="RESPONDENT_ENROLED", desc="Respondent enrolled"
        )
...
def register_user(party):
    ...
    if not oauth_response.status_code == 201:
        logger.info(
            "Registering respondent OAuth2 server responded with",
            status=oauth_response.status_code,
            content=oauth_response.content,
        )
        oauth_response.raise_for_status()
...
def request_case(enrolment_code):
    ...
    logger.info(
        "Successfully retrieved case from an enrolment code",
...
def request_collection_exercise(collection_exercise_id):
    ...
    logger.info(
        "Retrieving collection exercise by id",
        collection_exercise_id=collection_exercise_id,
    )
...
def request_survey(survey_id):
    ...
    logger.info(
        "Retrieving survey information from the survey service", survey_id=survey_id
    )
...
def request_casegroups_for_business(business_id):
    ...
    response.raise_for_status()
    logger.info(
        "Successfully retrieved casegroups for business", business_id=business_id
    )
    return response.json()
...
def request_collection_exercises_for_survey(survey_id):
    ...
    response.raise_for_status()
    logger.info(
        "Successfully retrieved collection exercises for survey", survey_id=survey_id
    )
    return response.json()
...
def get_business_survey_casegroups(survey_id, business_id):
    logger.info(
        "Retrieving casegroups for business and survey",
        survey_id=survey_id,
        business_id=business_id,
    )
    collection_exercise_ids = [
        ce["id"] for ce in request_collection_exercises_for_survey(survey_id)
    ]
    ...
    logger.info(
        "Successfully retrieved casegroups for business and survey",
        survey_id=survey_id,
        business_id=business_id,
    )
...
def get_case_id_for_business_survey(survey_id, business_id):
    logger.info(
        "Retrieving case for survey and business",
        survey_id=survey_id,
        business_id=business_id,
    )
