...
def share_survey_users():
    ...
    if business_id and survey_id:
        ...
        response = share_survey_controller.get_users_enrolled_and_pending_share_against_business_and_survey(
            business_id, survey_id
        )
...
def post_pending_shares():
    ...
    try:
        ...
        for pending_share in pending_shares:
            ...
            share_survey_controller.pending_share_create(
                business_id=pending_share["business_id"],
        ...
        send_pending_share_email(
            personalisation,
            email_template,
            pending_shares[0]["email_address"],
            batch_number,
        )
...
def send_pending_share_email(
    personalisation: dict, template: str, email: str, batch_id
):
    """
    ...
    try:
        ...
        logger.error(
            "Error sending sending email for survey share", batch_id=str(batch_id)
        )
...
@share_survey_view.route(
    "/share-survey/confirm-pending-shares/<batch_no>", methods=["POST"]
)
