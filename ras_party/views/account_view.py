...
@account_view.route(
    "/resend-account-email-change-expired-token/<token>", methods=["POST"]
)
def resend_account_email_change_expired_token(token):
    response = (
        account_controller.resend_account_email_change_verification_email_expired_token(
            token
        )
    )
    return make_response(jsonify(response), 200)
...
@account_view.route(
    "/resend-verification-email-expired-token/<token>", methods=["POST"]
)
...
def disable_user_enrolments():
    ...
    removed_enrolment_count = account_controller.disable_all_respondent_enrolments(
        email
    )

    return make_response(
        jsonify({"message": f"{removed_enrolment_count} enrolments removed"}), 200
    )
