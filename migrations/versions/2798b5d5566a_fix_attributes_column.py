...
def upgrade():
    ...
    for row in result:
        attributes = json.loads(json.dumps(row["attributes"])).replace("'", "''")  # Escape single quote & format json
        insert_sql = f"UPDATE partysvc.business_attributes " f"SET attributes = '{attributes}' WHERE id={row['id']}"
