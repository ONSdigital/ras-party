...
class TestRespondents(PartyTestClient):
    ...
    def test_get_respondents_using_end_of_firstname_does_not_return_respondent(self):
        ...
        mock_respondent.attributes(firstName="Andrew")
        self.populate_with_respondent(respondent=mock_respondent.as_respondent())
        response = self.get_respondents_by_name_email(first_name="rew", last_name=None, email=None, expected_status=200)
        self.assertEqual(len(response["data"]), 0)
    ...
    def test_get_respondents_using_end_of_lastname_does_not_return_respondent(self):
        ...
        mock_respondent.attributes(lastName="Torrance")
        self.populate_with_respondent(respondent=mock_respondent.as_respondent())
        response = self.get_respondents_by_name_email(first_name=None, last_name="nce", email=None, expected_status=200)
        self.assertEqual(len(response["data"]), 0)
    ...
    def test_get_respondents_using_first_and_last_name_only_returns_matching_respondent(self):
        ...
        mock_respondent1.attributes(
            firstName="Andrew", lastName="Torrance", emailAddress="Andrew.Torrance@something.com"
        )
    ...
    def test_get_respondents_using_matching_email_returns_respondent(self):
        ...
        response = self.get_respondents_by_name_email(
            first_name=None, last_name=None, email="Andrew.Smith@something.com", expected_status=200
        )
        self.assertEqual(len(response["data"]), 1)
        self.assertEqual(response["data"][0]["firstName"], "Andrew")
    ...
    def test_get_respondents_using_non_matching_email_returns_no_respondent(self):
        ...
        response = self.get_respondents_by_name_email(
            first_name=None, last_name=None, email="Andrew.Williams@something.com", expected_status=200
        )
        self.assertEqual(len(response["data"]), 0)
    ...
    def test_get_respondents_using_name_and_email_returns_only_the_respondent_that_match_all_params(self):
        ...
        mock_respondent3.attributes(
            firstName="Andrew", lastName="Williams", emailAddress="Andrew.Williamsh@something.com"
        )
    ...
    def test_get_respondents_using_name_and_email_returns_respondents_ordered_by_last_name_asc(self):
        mock_respondent1 = MockRespondent()
        mock_respondent1.attributes(firstName="Andrew", lastName="Gamma", emailAddress="Andrew.Torrance@something.com")
        self.populate_with_respondent(respondent=mock_respondent1.as_respondent())
    ...
    def test_get_respondents_limit_returns_the_selected_number_of_respondents(self):
        ...
        response = self.get_respondents_by_name_email(
            first_name="Andrew", last_name=None, email="Andrew", page=1, limit=6, expected_status=200
        )
    ...
    def test_get_respondents_page_returns_the_expected_respondents(self):
        ...
        for i in range(0, 26):
            mock_respondent.attributes(
                firstName="Andrew", lastName=respondents_last_name[i], emailAddress=f"Andrew_{i}.Torrance@something.com"
            )

            self.populate_with_respondent(respondent=mock_respondent.as_respondent())

        response = self.get_respondents_by_name_email(
            first_name=None, last_name=None, email="Andrew", page=3, limit=6, expected_status=200
        )
        self.assertEqual(len(response["data"]), 6)
    ...
    def test_get_respondents_page_returns_remaining_respondents_on_last_page(self):
        ...
        response = self.get_respondents_by_name_email(
            first_name=None, last_name=None, email="Andrew", page=3, limit=12, expected_status=200
        )
    ...
    def test_get_respondents_total_represents_total_matching_records(self):
        ...
        response = self.get_respondents_by_name_email(
            first_name="fifthAndrew", last_name=None, email=None, page=3, limit=2, expected_status=200
        )
        self.assertEqual(len(response["data"]), 2)
    ...
    def test_get_respondents_page_returns_zero_respondents_if_none_on_requested_page(self):
        ...
        response = self.get_respondents_by_name_email(
            first_name=None, last_name=None, email="Andrew", page=22, limit=12, expected_status=200
        )
        self.assertEqual(len(response["data"]), 0)
    ...
    def test_request_password_change_uses_case_insensitive_email_query():
        ...
        ) as db, patch("ras_party.controllers.account_controller.NotifyGateway"), patch(
    ...
    def test_notify_account_ras_notify_error(self):
        with patch("ras_party.controllers.account_controller.NotifyGateway") as notify, patch(
            "ras_party.controllers.account_controller.PublicWebsite"
        ):
    ...
    def test_post_respondent_creates_the_enrolment(self):
        ...
        self.assertEqual(str(enrolment.business_respondent.respondent.party_uuid), created_respondent["id"])
        # And is linked to the given business
        self.assertEqual(str(enrolment.business_respondent.business.party_uuid), DEFAULT_BUSINESS_UUID)
    ...
    def test_post_respondent_uses_case_insensitive_email_query(self):
        with patch("ras_party.controllers.queries.query_respondent_by_email") as query, patch(
            "ras_party.support.session_decorator.current_app.db"
        ) as db, patch("ras_party.controllers.account_controller.NotifyGateway"), patch(
    ...
    def test_post_add_new_survey_no_respondent_business_association(self):
        ...
        with responses.RequestsMock() as rsps:
            url_request_collection_exercises_for_survey = (
                f"{TestingConfig.CASE_URL}" f"/cases/7bc5d41b-0549-40b3-ba76-42f6d4cf3fdb/events"
            )
            rsps.add(rsps.POST, url_request_collection_exercises_for_survey, json={})
    ...
    def test_post_add_new_survey_missing_party_id_returns_error(self):
        ...
        token = self.generate_valid_token_from_email(db_respondent.email_address)
        self.put_email_verification(token, 200)
        request_json = {"enrolment_code": self.mock_respondent_with_id["enrolment_code"]}
    ...
    def test_disable_all_respondent_enrolments_disables_all_enrolments(self, mock_post_case, mock_get_case):
        ...
        with responses.RequestsMock() as rsps:
            url_request_collection_exercises_for_survey = (
                f"{TestingConfig.CASE_URL}" f"/cases/fdd36c0d-3037-4b80-9352-0da0f4acd777/events"
            )
            rsps.add(rsps.POST, url_request_collection_exercises_for_survey, json={})
    ...
    def _create_enrolments(self, second_enrolment_status):
        ...
        self.populate_with_respondent(respondent=self.mock_respondent_with_id)
        self.populate_with_business()
        self.associate_business_and_respondent(business_id=DEFAULT_BUSINESS_UUID, respondent_id=respondent_id)
        self.populate_with_enrolment()
    ...
    def test_put_change_respondent_account_status_active(self):
        respondent = self.populate_with_respondent(respondent=self.mock_respondent_with_id)
        self.populate_with_business()
        self.associate_business_and_respondent(business_id=DEFAULT_BUSINESS_UUID, respondent_id=respondent.party_uuid)
        enrolment = self.mock_enrolment_pending
        self.populate_with_enrolment(enrolment=enrolment)
        ...
        with responses.RequestsMock() as rsps:
            url_request_collection_exercises_for_survey = (
                f"{TestingConfig.CASE_URL}" f"/cases/f8d7a5db-2b72-4409-b4d2-bc47b358cbda/events"
            )
            rsps.add(rsps.POST, url_request_collection_exercises_for_survey, json={})
    ...
    def test_account_view_auth_error_calls_rollback(self):
        ...
        with patch("ras_party.controllers.account_controller.OauthClient") as auth, patch(
            "ras_party.controllers.account_controller.logger"
        ) as logger:
    ...
    def test_delete_respondent_by_email_sqlalchemyerror_on_commit(self):
        with patch("ras_party.controllers.queries.query_single_respondent_by_email") as query, patch(
            "ras_party.support.session_decorator.current_app.db"
        ) as db:
            query.return_value = {
