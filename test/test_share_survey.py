...
from test.test_data.mock_respondent import (MockNewRespondentWithId,
                                            MockRespondent,
...
from ras_party.controllers.queries import (
    query_business_by_party_uuid, query_pending_shares_by_business_and_survey,
    query_respondent_by_party_uuid)
from ras_party.models.models import (BusinessRespondent, Enrolment,
                                     PendingShares, Respondent,
                                     RespondentStatus)
...
class TestShareSurvey(PartyTestClient):
    ...
    def setUp(self):
        ...
        self.mock_respondent_test_with_id = (
            MockNewRespondentWithId().attributes().as_respondent()
        )
    ...
    def is_pending_survey_registered(self, business_id, survey_id, session):
        pending_survey = query_pending_shares_by_business_and_survey(
    ...
    def test_share_survey_users_with_no_pending_share(self):
        ...
        # When
        response = self.get_share_survey_users(
            DEFAULT_BUSINESS_UUID, DEFAULT_SURVEY_UUID
        )
        # Then
    ...
    def test_share_survey_users_with_pending_share_bad_request(self):
        ...
        self.assertEqual(
            ...
            "Business id and Survey id is required for this request.",
        )
    ...
    def test_share_survey_users_with_pending_share_not_found(self):
        ...
        self.populate_pending_share()
        # When
        response = self.get_share_survey_users_not_found(
            "3b136c4b-7a14-4904-9e01-13364dd7b973", DEFAULT_SURVEY_UUID
        )
        # Then
        self.assertEqual(
            response["description"], "Business with party id does not exist"
        )
    ...
    def test_post_pending_shares_success(self):
        ...
        with patch(
            "ras_party.views.share_survey_view.send_pending_share_email"
        ) as pending_share_email:
            response = self.post_share_surveys(payload)
            # Then
            self.assertEqual(response, {"created": "success"})
            self.assertTrue(
                self.is_pending_survey_registered(
                    DEFAULT_BUSINESS_UUID, DEFAULT_SURVEY_UUID
                )
            )
            self.assertTrue(
                self.is_pending_survey_registered(
                    DEFAULT_BUSINESS_UUID, "cb0711c3-0ac8-41d3-ae0e-567e5ea1ef99"
                )
            )
    ...
    def test_post_pending_shares_fail_invalid_payload(self):
        ...
        self.assertEqual(
            response["description"], "Payload Invalid - Pending share key missing"
        )
    ...
    def test_post_pending_shares_fail_invalid_payload_size(self):
        ...
        response = self.post_share_surveys_fail(payload)
        self.assertEqual(
            response["description"], "Payload Invalid - pending_shares list is empty"
        )
    ...
    def test_post_pending_shares_fail_validation(self):
        ...
        response = self.post_share_surveys_fail(payload)
        self.assertEqual(
            response["description"], ["Required key 'email_address' is missing."]
        )
    ...
    def test_delete_pending_shares_success(self):
        ...
        with patch(
            "ras_party.views.batch_request.send_pending_share_email"
        ) as pending_share_email:
    ...
    def test_share_survey_verification_token_success(self):
        ...
        response = self.verify_share_surveys(
            generate_email_token(str(self.mock_pending_share["batch_no"]))
        )
        self.assertEqual(
            response[0]["batch_no"], str(self.mock_pending_share["batch_no"])
        )
    ...
    def test_share_survey_verification_token_fail(self):
        ...
        response = self.verify_share_surveys(
            generate_email_token(str(self.mock_pending_share["batch_no"])), 404
        )
