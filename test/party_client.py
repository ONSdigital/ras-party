...
class PartyTestClient(TestCase):
    ...
    def create_app():
        ...
        app.db = create_database(
            app.config["DATABASE_URI"], app.config["DATABASE_SCHEMA"]
        )
    ...
    def tearDown(self):
        ...
        connection.execute(
            f"drop schema {current_app.config['DATABASE_SCHEMA']} cascade;"
        )
        connection.close()
    ...
    def post_to_parties(self, payload, expected_status):
        ...
        self.assertStatus(
            response,
            expected_status,
            "Response body is : " + response.get_data(as_text=True),
        )
        return json.loads(response.get_data(as_text=True))
    ...
    def get_party_by_ref(self, party_type, ref, expected_status=200):
        response = self.client.get(
            f"/party-api/v1/parties/type/{party_type}/ref/{ref}",
            headers=self.auth_headers,
        )
        self.assertStatus(
            response,
    ...
    def get_business_by_id(self, id, expected_status=200, query_string=None):
        response = self.client.get(
            f"/party-api/v1/businesses/id/{id}",
    ...
    def get_business_by_ref(self, ref, expected_status=200, query_string=None):
        response = self.client.get(
            f"/party-api/v1/businesses/ref/{ref}",
            query_string=query_string,
            headers=self.auth_headers,
        )
        self.assertStatus(
    ...
    def get_respondents_by_name_email(
        self, first_name, last_name, email, page=1, limit=10, expected_status=200
    ):
    ...
    def get_respondent_by_id(self, id, expected_status=200):
        response = self.client.get(
            f"/party-api/v1/respondents/id/{id}", headers=self.auth_headers
        )
        self.assertStatus(
            response,
            expected_status,
    ...
    def put_email_verification(self, token, expected_status):
        response = self.client.put(
            f"/party-api/v1/emailverification/{token}", headers=self.auth_headers
        )
        self.assertStatus(
            response,
            expected_status,
    ...
    def resend_verification_email(self, party_uuid, expected_status=200):
        response = self.client.post(
            f"/party-api/v1/resend-verification-email/{party_uuid}",
            headers=self.auth_headers,
        )
        self.assertStatus(
            response,
    ...
    def resend_verification_email_expired_token(self, token, expected_status=200):
        response = self.client.post(
            f"/party-api/v1/resend-verification-email-expired-token/{token}",
            headers=self.auth_headers,
        )
        self.assertStatus(
            response,
    ...
    def resend_account_email_change_expired_token(self, token, expected_status=200):
        response = self.client.post(
            f"/party-api/v1/resend-account-email-change-expired-token/{token}",
            headers=self.auth_headers,
        )
        self.assertStatus(
            response,
    ...
    def resend_password_email_expired_token(self, token, expected_status=200):
        response = self.client.post(
            f"/party-api/v1/resend-password-email-expired-token/{token}",
            headers=self.auth_headers,
        )
        self.assertStatus(
            response,
    ...
    def verify_token(self, token, expected_status=200):
        response = self.client.get(
            f"/party-api/v1/tokens/verify/{token}", headers=self.auth_headers
        )
        self.assertStatus(
            response,
            expected_status,
    ...
    def get_businesses_search(
        self, expected_status=200, query_string=None, page=1, limit=100
    ):
        response = self.client.get(
            f"/party-api/v1/businesses/search?"
    ...
    def patch_disable_all_respondent_enrolments(
        self, email_address, expected_status=200
    ):
    ...
    def delete_user_data_marked_for_deletion(self, expected_status=204):
        response = self.client.delete(
            f"/party-api/v1/batch/respondents", headers=self.auth_headers
        )
        self.assertStatus(response, expected_status)
        return response
    ...
    def get_share_survey_users(self, business_id, survey_id, expected_status=200):
        ...
        response = self.client.get(
            f"/party-api/v1/share-survey-users-count",
            query_string=data,
            headers=self.auth_headers,
        )
        self.assertStatus(
    ...
    def get_share_survey_users_not_found(
        self, business_id, survey_id, expected_status=404
    ):
        data = {
            "business_id": business_id,
            "survey_id": survey_id,
    ...
    def get_share_survey_users_bad_request(self, expected_status=400):
        response = self.client.get(
            f"/party-api/v1/share-survey-users-count", headers=self.auth_headers
        )
        self.assertStatus(
            response,
            expected_status,
    ...
    def post_share_surveys(self, payload, expected_status=201):
        response = self.client.post(
            f"/party-api/v1/pending-shares", json=payload, headers=self.auth_headers
        )
        self.assertStatus(
            response,
            expected_status,
    ...
    def delete_share_surveys(self, expected_status=204):
        response = self.client.delete(
            f"/party-api/v1/batch/pending-shares", headers=self.auth_headers
        )
        self.assertStatus(response, expected_status)
    ...
    def verify_share_surveys(self, token, expected_status=200):
        ...
        response = self.client.get(
            f"/party-api/v1/share-survey/verification/{token}",
            headers=self.auth_headers,
        )
        self.assertStatus(
            response,
    ...
    def confirm_share_survey(self, batch_no, expected_status=201):
        self.app.config["EMAIL_TOKEN_EXPIRY"] = 36000
        response = self.client.post(
            f"/party-api/v1/share-survey/confirm-pending-shares/{batch_no}",
            headers=self.auth_headers,
        )
